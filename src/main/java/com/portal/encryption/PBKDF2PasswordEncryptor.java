...
public class PBKDF2PasswordEncryptor {
	...
	public String doEncrypt(String algorithm, String plainTextPassword, String encryptedPassword) throws Exception {
		...
		try {
			...
			return String.valueOf(pbkdf2EncryptionConfiguration.getKeySize()) + "_" + pbkdf2EncryptionConfiguration.getRounds() + "_" + DatatypeConverter.printBase64Binary(saltBytes) + "_" + DatatypeConverter.printBase64Binary(secretKeyBytes);
		} catch (InvalidKeySpecException | NoSuchAlgorithmException e) {
			throw new Exception("Internal error while hashing password", e);
		}
	}
		...
		public void configure(String algorithm, String encryptedPassword) {
			if (StringUtils.isEmpty(encryptedPassword)) {
				...
				if (matcher.matches()) {
					...
					BigEndianCodec.putLong(_saltBytes, 0, new BigInteger(DatatypeConverter.parseBase64Binary(matcher.group(3))).longValue());
				}
			}
		}
